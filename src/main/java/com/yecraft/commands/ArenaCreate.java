	...
	public void init(String[] args, Player player) {
		if (args[0].equalsIgnoreCase("create") & args.length == 2){
			...
			if (!Arena.ARENA_MAP.containsKey(name)){
				...
				BossBar bossBar = Bukkit.createBossBar("Очікування гравців...", BarColor.BLUE, BarStyle.SOLID);
				Arena arena = new Arena(name, 0, 0, game, false, new LocalGameMap("test",true), bossBar, new HashSet<>(), new HashMap<UUID, Location>());
